...
// This can be solved by Monte Carlo simulation, but I chose to approach it analytically as a
...
class Problem84 extends Problem(84, solution = "101524") {
  ...
  val P: List[List[Double]] = {
    ...
    val rolls = for(die1 <- 1 to die; die2 <- 1 to die)
                yield die1 + die2
    def row(i: Int) =
  ...
  def solve = {
    val stationaryProbabilityVector =
      Iterator.iterate(P)(matrixMul(_, P)).drop(150).next.head
