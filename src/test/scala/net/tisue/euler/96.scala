...
// The input file contains fifty different Sudoku puzzles ranging in difficulty,
// but all with unique solutions.  By solving all fifty puzzles find the sum of
// the 3-digit numbers found in the top left corner of each solution grid (483
// for the first puzzle).

// Each puzzle is a list of 81 lists of integers 1-9, each list representing
// possibilities which have not yet been ruled out.  The puzzle is solved when
// all the possibility lists have length 1.  Each group (row, column, or 3x3
// box) is a list of 9 indices into the puzzle.

// I did not read the Python code at
// http://www.eddaardvark.co.uk/sudokusolver.html, but I did more or less follow
// the suggested algorithm.

// Note that in that web page's terminology, puzzle #1 is "trivial" and puzzles
// #2 through #5 are "easy", but some subsequent puzzles are neither.  solve1
// corresponds to the "Applying the given numbers" section, solve2 corresponds
// to the "Looking for Singletons" section, I skipped the "Pairs" part, and
...
class Problem96 extends Problem(96, "24702"):
  ...
  // like I've already spent enough time on this.  The basic idea is that after
  // solve2 makes a change, solve1 must be run to exhaustion, and after solve3
