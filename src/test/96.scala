...
class Problem96 extends Problem(96, "24702") {
  def solve = {
    ...
    def nAtATime[T](size:Int,items:Iterator[T]):List[List[T]] =
      if(!items.hasNext) Nil
      else items.take(size).toList :: nAtATime(size,items)
    val puzzles:List[Puzzle] =
      nAtATime(9,io.Source.fromFile("dat/96.txt").getLines.filter(!_.startsWith("Grid")))
        .map(_.mkString.view.filter(_.isDigit).map(_.asDigit).toList
              .map(d => if(d == 0) (1 to 9).toList else List(d)))
    ...
    def replaceItem[T](l:List[T],i:Int,newValue:T) = l.take(i) ::: newValue :: l.drop(i + 1)
    def repeat[A](x:A,fn:(A)=>A):A = { val x2 = fn(x); if(x == x2) x else repeat(x2,fn) }
    ...
    def solve1(puzzle:Puzzle) =
      groups.foldLeft(puzzle){(puzzle,group) =>
        ...
        puzzle.zipWithIndex.map{case (entry,index) => if(!group.contains(index) || entry.size == 1) entry
                                                      else entry.filter(!usedDigits.contains(_))}}
    def solve2(puzzle:Puzzle) =
      groups.foldLeft(puzzle){(puzzle,group) =>
        val v = (1 to 9).filter(d => group.filter(puzzle(_).size > 1).flatMap(puzzle(_)).count(_ == d) == 1)
        v.foldLeft(puzzle){(puzzle,d) =>
          repeat(puzzle.zipWithIndex.map{case (entry,index) => if(group.contains(index) && entry.contains(d)) List(d)
                                                               else entry},
                 solve1)}}
    def solve1and2(puzzle:Puzzle) = repeat(repeat(puzzle,solve1),solve2)
    def isValid(puzzle:Puzzle) = groups.forall(g => g.flatMap(puzzle(_)).toSet.size == 9)
    def solve3(puzzle:Puzzle):Option[Puzzle] =
      if(!isValid(puzzle)) None
      else puzzle.indices.find(puzzle(_).size > 1) match {
             case None => Some(puzzle)
             case Some(index) =>
               puzzle(index).flatMap{guess => solve3(repeat(replaceItem(puzzle,index,List(guess)),
                                                            solve1and2))}
                            .headOption
           }
