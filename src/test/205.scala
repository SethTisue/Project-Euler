...
class Problem205 extends Problem(205, "0.5731441") {
  def solve = {
    def rolls(n:Int,sides:Int):List[Int] =
      if(n == 0) List(0)
      else (1 to sides).toList.flatMap(roll => rolls(n - 1, sides).map(_ + roll))
    def counts[T](ns:List[T]):Map[T,Int] = {
      def increment(map:Map[T,Int],key:T) = map.updated(key, map(key) + 1)
    ...
    def answer(n1:Int,sides1:Int,n2:Int,sides2:Int) = {
      val wins1 =
        for{(sum1,count1) <- counts(rolls(n1,sides1))
            ...
            if sum1 > sum2}
        yield count1 * count2
      wins1.map(_.toLong).sum /
        (math.pow(sides1,n1) * math.pow(sides2,n2))
    }
    "%.7f".format(answer(9,4,6,6))
  }
}
